...
Inventory = {
    ...
    clearInventory = function(inventory, keep)
        if type(inventory) == 'string' then
            exports['ps-inventory']:ClearStash(inventory)
        else
            exports['ps-inventory']:ClearInventory(inventory)
        end
    end,
    ...
    registerShop = function(shopName, shopData)
        for i = 1, #shopData.inventory, 1 do
            shopData.inventory[i].amount = 999
            shopData.inventory[i].slot = i
    ...
    addStashItem = function(stashData, itemName, itemCount, metadata, slot)
        exports['ps-inventory']:AddItem(stashData.id..'_'..stashData.owner, itemName, itemCount, slot, metadata)
    end,
}
...
RegisterNetEvent('p_policejob/inventory/openInventory', function(invType, data)
    ...
    elseif invType == 'player' then
        exports['ps-inventory']:OpenInventory('otherplayer', data, nil, source)
    else
