...
Core = {
    ...
    GetPlayerGroup = function(playerId)
        local permissions = exports['qbx_core']:GetPermission(playerId)
    ...
    ReducePlayersJail = function()
        ...
        local data = {}
        for _, playerJail in pairs(playersJail) do
    ...
    RemoveVehicleFromImpound = function(playerId, data)
        ...
        if values.vin then
            table.insert(queries, 'INSERT INTO player_vehicles (license, citizenid, vehicle, hash, mods, plate, vin, state) VALUES (@license, @owner, @vehicle, @hash, @mods, @plate, @vin, 0)')
        else
            table.insert(queries, 'INSERT INTO player_vehicles (license, citizenid, vehicle, hash, mods, plate, state) VALUES (@license, @owner, @vehicle, @hash, @mods, @plate, 0)')
        end
