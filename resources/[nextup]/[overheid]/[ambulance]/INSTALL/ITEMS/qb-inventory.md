	crutch = {
		name = 'crutch',
		label = 'Crutch',
		weight = 100,
		type = 'item',
		unique = true,
		shouldClose = true,
		image = 'crutch.png',
		useable = true
	},

	wheelchair = {
		name = 'wheelchair',
		label = 'Wheelchair',
		weight = 100,
		type = 'item',
		unique = true,
		shouldClose = true,
		image = 'wheelchair.png',
		useable = true
	},

	stretcher = {
		name = 'stretcher',
		label = 'Stretcher',
		weight = 100,
		unique = true,
		shouldClose = true,
		image = 'stretcher.png',
		useable = true
	},

	medical_kit = {
		name = 'medical_kit',
		label = 'Medical Kit',
		weight = 200,
		type = 'item',
		unique = true,
		shouldClose = true,
		image = 'medical_kit.png',
		useable = true,
		description = 'A basic medical kit containing essential supplies for treating minor injuries and illnesses.',
	},

	advanced_medical_kit = {
		name = 'advanced_medical_kit',
		label = 'Advanced Medical Kit',
		weight = 200,
		type = 'item',
		unique = true,
		shouldClose = true,
		image = 'advanced_medical_kit.png',
		useable = true,
		description = 'A more advanced medical kit containing additional supplies and equipment for treating injuries and illnesses.',
	},

	blood_bag_250 = {
		name = 'blood_bag_250',
		label = 'Blood Bag 250ml',
		weight = 250,
		type = 'item',
		unique = false,
		shouldClose = false,
		image = 'blood_bag_250.png',
		useable = false,
		description = 'A 250ml bag of blood used for blood transfusions.',
	},

	blood_bag_500 = {
		name = 'blood_bag_500',
		label = 'Blood Bag 500ml',
		weight = 500,
		type = 'item',
		unique = false,
		shouldClose = false,
		image = 'blood_bag_500.png',
		useable = false,
		description = 'A 500ml bag of blood used for blood transfusions.',
	},

	painkillers = {
		name = 'painkillers',
		label = 'Painkillers',
		weight = 50,
		type = 'item',
		unique = false,
		shouldClose = false,
		image = 'painkillers.png',
		useable = false,
		description = 'A medication used to relieve pain and reduce fever.',
	},

	adrenaline = {
		name = 'adrenaline',
		label = 'Adrenaline',
		weight = 50,
		type = 'item',
		unique = false,
		shouldClose = false,
		image = 'adrenaline.png',
		useable = true,
	},

	morphine = {
		name = 'morphine',
		label = 'Morphine',
		weight = 50,
		type = 'item',
		unique = false,
		shouldClose = false,
		image = 'morphine.png',
		useable = false,
		description = 'A medication used to relieve pain and reduce fever.',
	},

	suture_kit = {
		name = 'suture_kit',
		label = 'Suture Kit',
		weight = 100,
		type = 'item',
		unique = false,
		shouldClose = false,
		image = 'suture_kit.png',
		useable = false,
		description = 'A medical device used to close wounds or surgical incisions.',
	},

	icepack = {
		name = 'icepack',
		label = 'Ice Pack',
		weight = 100,
		type = 'item',
		unique = false,
		shouldClose = false,
		image = 'icepack.png',
		useable = false,
		description = 'A bag of ice used to reduce swelling and numb pain.',
	},

	splint = {
		name = 'splint',
		label = 'Splint',
		weight = 100,
		type = 'item',
		unique = false,
		shouldClose = false,
		image = 'splint.png',
		useable = false,
		description = 'A device that is used to apply pressure to a limb.',
	},

	defibrilator = {
		name = 'defibrilator',
		label = 'Defibrillator',
		weight = 500,
		type = 'item',
		unique = true,
		shouldClose = true,
		image = 'defibrilator.png',
		useable = true,
	},

	bodybag = {
		name = 'bodybag',
		label = 'Body Bag',
		weight = 500,
		type = 'item',
		unique = false,
		shouldClose = false,
		image = 'bodybag.png',
		useable = false,
	},

	gauze = {
		name = 'gauze',
		label = 'Gauze',
		weight = 20,
		type = 'item',
		unique = false,
		shouldClose = true,
		image = 'gauze.png',
		useable = true,
		description = 'A thin, transparent fabric with a loose open weave, used for dressings, bandages, and surgical sponges.',
	},

	bandage = {
		name = 'bandage',
		label = 'Bandage',
		description = 'Very good for stopping bleeding and small injuries',
		weight = 115,
		type = 'item',
		unique = false,
		shouldClose = true,
		image = 'bandage.png',
		useable = true
	},

	ointment = {
		name = 'ointment',
		label = 'Ointment',
		weight = 50,
		type = 'item',
		unique = false,
		shouldClose = false,
		image = 'ointment.png',
		useable = false,
		description = 'A medical cream used to promote healing and prevent infection in minor cuts, scrapes, and burns.',
	},

	disinfectant = {
		name = 'disinfectant',
		label = 'Disinfectant',
		weight = 50,
		type = 'item',
		unique = false,
		shouldClose = false,
		image = 'disinfectant.png',
		useable = false,
		description = 'A liquid that kills bacteria and other microorganisms on surfaces.',
	},

	cyclonamine = {
		name = 'cyclonamine',
		label = 'Cyclonamine',
		weight = 50,
		type = 'item',
		unique = false,
		shouldClose = true,
		image = 'cyclonamine.png',
		useable = true,
	},

	tourniquet = {
		name = 'tourniquet',
		label = 'Tourniquet',
		weight = 100,
		type = 'item',
		unique = false,
		shouldClose = true,
		image = 'tourniquet.png',
		useable = true,
		description = 'A device that is used to apply pressure to a limb.',
	},

	medicbag = {
		name = 'medicbag',
		label = 'Medic Bag',
		weight = 500,
		type = 'item',
		unique = true,
		shouldClose = true,
		image = 'medicbag.png',
		useable = true,
		description = 'A bag containing medical supplies and equipment.',
	},

	antipyretics = {
		name = 'antipyretics',
		label = 'Antipyretics',
		weight = 50,
		type = 'item',
		unique = false,
		shouldClose = false,
		image = 'antipyretics.png',
		useable = false,
		description = 'A medication that reduces fever.',
	},

	ambulance_gps = {
		name = 'ambulance_gps',
		label = 'Ambulance GPS',
		weight = 100,
		type = 'item',
		unique = true,
		shouldClose = true,
		image = 'ambulance_gps.png',
		useable = true,
	},