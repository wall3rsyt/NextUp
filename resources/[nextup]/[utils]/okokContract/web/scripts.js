...
// Sanitization function to prevent XSS in signatures and other user input
function sanitizeInput(input) {
    if (typeof input !== 'string') return input;
    // Remove HTML tags and dangerous characters that could be used for injection
...
window.addEventListener('message', function(event) {
	switch (event.data.action) {
		case 'openContractSeller':
			...
			$("#source_name").text(event.data.source_playername);
			$("#target_name").text(event.data.target_playername);
			$("#vehicle_model").text(event.data.model);
			$("#plate_number").text(event.data.plate);
			$("#date").text(event.data.date);
		...
		case 'openContractInfo':
			...
			$("#vehicle_name").text(event.data.model);
			$("#vehicle_plate").text(event.data.plate);
		...
		case 'openContractOnBuyer':
			...
			var sellerSignatureP = new Vara("#sign1","SatisfySL.json", [{
				text: sanitizeInput(event.data.source_playername),
				fontSize: 20, 
				strokeWidth: 2,
				color: "#000",
				id: "",
...
$(document).on('click', '#submitContractInfo', function() {
	...
	vehicle_price = parseFloat(vehicle_price) || 0;
	player_dropdown = parseInt(player_dropdown) || 0;
...
$(document).on('click', "#signContract2", function() {
	var buyerSignature = new Vara("#sign2","SatisfySL.json", [{
		text: sanitizeInput(targetNameSeller),
		fontSize: 20, 
		strokeWidth: 2,
		color: "#000",
		id: "",
