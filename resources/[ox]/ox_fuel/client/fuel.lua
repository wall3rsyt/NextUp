...
function fuel.startFueling(vehicle, isPump)
	...
	-- Jobs die gratis tanken
	local freeJobs = { ambulance = true, police = true, mechanic = true }
	local playerJob = exports.qbx_core:GetPlayerData().job.name or ''
	local isFreeJob = freeJobs[playerJob] or false
	...
	if isPump then
		price = 0
		if not isFreeJob then
			local input = lib.inputDialog('Tanken', {
				{ type = 'select', label = 'Betaalmethode', options = {
					{ value = 'cash', label = 'Cash' },
					{ value = 'bank', label = 'Bank' },
				}, required = true }
			})
			if not input then return end
			state.paymentMethod = input[1]

			if state.paymentMethod == 'cash' then
	...
	if isPump then
		TriggerServerEvent('ox_fuel:pay', price, fuelAmount, NetworkGetNetworkIdFromEntity(vehicle), isFreeJob and 'free' or state.paymentMethod)
	else
