...
-- Cache natives and globals used in hot paths
local Wait = Wait
local pairs = pairs
local type = type
local next = next
...
local IsPedPerformingMeleeAction = IsPedPerformingMeleeAction
...
local SetAmmoInClip = SetAmmoInClip
...
local AddAmmoToPed = AddAmmoToPed
...
local TaskTurnPedToFaceCoord = TaskTurnPedToFaceCoord
...
local function closeTrunk()
	if currentInventory?.type ~= 'trunk' then return end
...
-- Shared NUI inventory setup helper to avoid duplication
local function setupInventoryNUI(left, right)
	left.items = PlayerData.inventory
	left.groups = PlayerData.groups
	SendNUIMessage({
		action = 'setupInventory',
		data = { leftInventory = left, rightInventory = right }
	})
end
...
function client.openInventory(inv, data)
		...
		if IsNuiFocused() then
			...
			if inv ~= 'drop' and inv ~= 'container' then
				if (data?.id or data) == currentInventory?.id then
					return warn((string_format("script tried to open inventory, but it is already open\n%s", Citizen.InvokeNative(`FORMAT_STACK_TRACE` & 0xFFFFFFFF, nil, 0, Citizen.ResultAsString()))))
				else
					return client.closeInventory()
				end
			end
	...
	if inv == 'player' and data ~= cache.serverId then
		...
		if not targetCoords
		or #(targetCoords - GetEntityCoords(playerPed)) > 1.8
		or (not client.hasGroup(shared.police) and not Player(serverId).state.canSteal) then
			return lib.notify({ id = 'inventory_right_access', type = 'error', description = locale('inventory_right_access') })
		end
	end
	...
	elseif invOpen ~= nil then
		if inv == 'policeevidence' and not data then
	...
	setupInventoryNUI(left, currentInventory)
...
-- Shared helper to open a forced/view-only inventory overlay
local function openOverlayInventory(left, right, extraFields)
...
RegisterNetEvent('ox_inventory:forceOpenInventory', function(left, right)
	openOverlayInventory(left, right, { ignoreSecurityChecks = true })
end)

RegisterNetEvent('ox_inventory:viewInventory', function(left, right)
	openOverlayInventory(left, right, { ignoreSecurityChecks = true, type = 'inspect' })
end)
...
lib.callback.register('ox_inventory:usingItem', function(data, noAnim)
	...
	if not item or not usingItem then return end
...
local function useSlot(slot, noAnim)
	...
	-- Naive durability check for early exit
	...
	if data.weapon then
		...
		if data.hash ~= GetSelectedPedWeapon(playerPed) then
			lib.print.info((string_format('failed to equip %s (cause unknown)', item.name)))
	...
	if currentWeapon then
		if data.ammo then
			...
			useItem(data, function(resp)
				...
				if currentWeapon.metadata.specialAmmo ~= resp.metadata.type and type(currentWeapon.metadata.specialAmmo) == 'string' then
					local clipComponentKey = string_format('COMPONENT_%s', Items[currentWeapon.name].model:gsub('WEAPON_', ''))
					local specialClip = string_format('%s_%s', clipComponentKey, (resp.metadata.type or currentWeapon.metadata.specialAmmo):upper())
					...
					if type(resp.metadata.type) == 'string' then
						if not HasPedGotWeaponComponent(playerPed, currentWeapon.hash, specialClip) then
							...
							local defaultClip = string_format('%s_01', clipComponentKey)
			...
			for i = 1, #components do
				...
				if DoesWeaponTakeWeaponComponent(currentWeapon.hash, component) then
					if HasPedGotWeaponComponent(playerPed, currentWeapon.hash, component) then
						...
						useItem(data, function(itemData)
							if itemData then
								local success = lib.callback.await('ox_inventory:updateWeapon', false, 'component', tostring(itemData.slot), currentWeapon.slot)

								if success then
									GiveWeaponComponentToPed(playerPed, currentWeapon.hash, component)
									TriggerEvent('ox_inventory:updateWeaponComponent', 'added', component, itemData.name)
								end
							end
						end)
					end
		...
		elseif data.allowArmed then
			...
			lib.notify({ id = 'cannot_perform', type = 'error', description = locale('cannot_perform') })
		end

		return
	end

	if not data.ammo and not data.component then
		useItem(data)
	end
end
...
local function useButton(id, slot)
	if not PlayerData.loaded or invBusy or lib.progressActive() then return end
...
local function registerCommands()
	...
	local primary = lib.addKeybind({
		...
		onPressed = function()
			...
			if cache.vehicle then return openGlovebox(cache.vehicle) end
	...
	lib.addKeybind({
		...
		onPressed = function(self)
			if primary:getCurrentKey() == self:getCurrentKey() then
				return warn((string_format("secondary inventory keybind '%s' disabled (keybind cannot match primary inventory keybind)", self:getCurrentKey())))
			end
			...
			if StashTarget then return client.openInventory('stash', StashTarget) end
			...
			if not shared.target and entityType == 3 then
				if Inventory.Dumpsters:includes(GetEntityModel(entity)) then
		...
		onPressed = function()
			...
			if currentWeapon.ammo then
				if currentWeapon.metadata.durability > 0 then
					...
					if slotId then useSlot(slotId) end
				else
	...
	for itemName, count in pairs(itemCount) do
		...
		if item then
			...
			if count < 0 then
				...
				if item.client?.remove then item.client.remove(item.count) end
			elseif count > 0 then
				if shared.framework == 'esx' then
					TriggerEvent('esx:addInventoryItem', item.name, item.count)
				end

				if item.client?.add then item.client.add(item.count) end
			end
		end
	end
...
local function nearbyDrop(point)
	if not point.instance or point.instance == currentInstance then
		DrawMarker(
			...
			client.dropmarker.colour[1], client.dropmarker.colour[2], client.dropmarker.colour[3],
...
RegisterNetEvent('ox_inventory:createDrop', function(dropId, data, owner, slot)
	...
	if owner ~= cache.serverId then return end
	...
	if #(GetEntityCoords(playerPed) - data.coords) <= 1 then
		if not cache.vehicle then
...
lib.onCache('seat', function(seat)
	if seat then
		...
		if hasWeapon then return Utils.WeaponWheel(true) end
	end
...
RegisterNetEvent('ox_inventory:setPlayerInventory', function(currentDrops, inventory, weight, player)
	...
	setmetatable(PlayerData, {
		__index = function(self, key)
			if key == 'ped' then return PlayerPedId() end
		end
	})

	if setStateBagHandler then setStateBagHandler(string_format('player:%s', cache.serverId)) end
	...
	for _, data in pairs(inventory) do
		...
		if item then
			...
			if add then add(item.count) end
		end
	end
	...
	-- Only extract UI-relevant locales once
	local uiLocales = {}
	...
	for id, data in pairs(lib.load('data.licenses') or {}) do
		lib.points.new({
			...
			message = string_format('**%s**  \n%s', locale('purchase_license', data.name), locale('interact_prompt', GetControlInstructionalButton(0, 38, true):sub(3)))
		})
	end
	...
	-- Cache frequently used tick-loop values as locals
	...
	local IsPedPlantingBomb = IsPedPlantingBomb
			...
			if not canOpenInventory() then
				...
				if currentInventory and not currentInventory.ignoreSecurityChecks then
					...
					if currentInventory.type == 'otherplayer' then
						...
						if not id
						or #(playerCoords - pedCoords) > maxDistance
						or (not client.hasGroup(shared.police) and not Player(currentInventory.id).state.canSteal) then
							client.closeInventory()
		...
		if currentWeapon then
			if weaponHash ~= currentWeapon.hash and currentWeapon.timer then
				...
				if weaponHash ~= currentWeapon.hash then
					lib.print.info(string_format('%s was forcibly unequipped (caused by game behaviour or another resource)', currentWeapon.name))
					currentWeapon = Weapon.Disarm(currentWeapon, true)
				end
			end
					...
					if IsPedShooting(playerPed) then
						...
						if currentWeapon.group == `GROUP_PETROLCAN` or currentWeapon.group == `GROUP_FIREEXTINGUISHER` then
							...
							if currentAmmo < weaponAmmo then
								...
								currentWeapon.metadata.durability -= durabilityDrain * math_abs((weaponAmmo or 0.1) - currentAmmo)
							end
						end

						if currentAmmo <= 0 then
							if cache.vehicle then TaskSwapWeapon(playerPed, true) end
							currentWeapon.timer = GetGameTimer() + 200
						else
							currentWeapon.timer = GetGameTimer() + (GetWeaponTimeBetweenShots(currentWeapon.hash) * 1000) + 100
						end
					end
	...
	if client.giveplayerlist then
		...
		for i = 1, #nearbyPlayers do
			...
			if isGiveTargetValid(option.ped, option.coords) then
				...
				option.label = string_format('[%s] %s', option.id, playerName)
				n += 1
				giveList[n] = option
			end
		end
...
RegisterNUICallback('swapItems', function(data, cb)
	...
	if currentWeapon and data.fromType ~= data.toType then
		if (data.fromType == 'player' and data.fromSlot == currentWeapon.slot)
		or (data.toType == 'player' and data.toSlot == currentWeapon.slot) then
			currentWeapon = Weapon.Disarm(currentWeapon, true)
		end
	end
...
RegisterNUICallback('buyItem', function(data, cb)
	...
	if data then
		...
		if data[3] then
			SendNUIMessage({
				action = 'refreshSlots',
				data = { items = { { item = data[3], inventory = 'shop' } } }
			})
		end
	end

	if message then lib.notify(message) end
