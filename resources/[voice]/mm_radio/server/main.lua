...
if Shared.UseCommand then
    lib.addCommand('radio', {
        ...
        for i=1, #Shared.RadioItem do
            if exports.ox_inventory:GetItemCount(source, Shared.RadioItem[i]) > 0 then
                hasRadio = true
                break
            end
        end
        if not hasRadio then
            TriggerClientEvent('ox_lib:notify', source, {
                type = 'error',
                description = 'Je hebt geen radio bij je.'
            })
            return
        end
    ...
    lib.addCommand('jammer', {
        ...
        if exports.ox_inventory:GetItemCount(source, 'jammer') <= 0 then
            TriggerClientEvent('ox_lib:notify', source, {
                type = 'error',
                description = 'Je hebt geen jammer bij je.'
            })
            return
        end
