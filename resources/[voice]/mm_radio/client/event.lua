local function openRadio(slot)
    ...
    local battery, radioId = lib.callback.await('mm_radio:server:getradiodata', false, slot or false)
    if not battery then battery = 100 end
    if not radioId then radioId = 'PERSONAL' end
...
RegisterNetEvent('mm_radio:client:use', openRadio)
AddEventHandler('mm_radio:client:use', openRadio)
...
RegisterNetEvent('mm_radio:client:usejammer', function()
    if not Shared.Jammer.permission or not (lib.table.contains(Shared.Jammer.permission, Radio.PlayerJob) or lib.table.contains(Shared.Jammer.permission, Radio.PlayerGang)) then
        lib.notify({
            title = 'Je hebt geen toestemming om dit item te gebruiken',
            type = 'error'
        })
        return
    end
