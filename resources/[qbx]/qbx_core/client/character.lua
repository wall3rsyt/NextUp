...
if config.characters.limitNationalities then
    ...
    -- FIX: geen onnodige thread, synchroon uitvoeren is sneller en simpeler
...
local function setupPreviewCam()
    ...
    -- FIX: Wait(0) elke frame was onnodig zwaar, 100ms is meer dan genoeg voor DoF
...
-- FIX: wacht tot kritieke resources 'started' zijn voordat events gefired worden
-- Voorkomt dat ox_inventory, ox_target, qb-clothes etc. nog niet klaar zijn bij spawn
local function waitForResources(resources)
    for _, resource in ipairs(resources) do
        local timeout = 0
        while GetResourceState(resource) ~= 'started' and timeout < 100 do
            Wait(100)
            timeout = timeout + 1
        end
    end
...
local function spawnDefault()
    ...
    -- FIX: wacht op inventory, target en clothes voordat de player echt ingeladen wordt
    waitForResources({'ox_inventory', 'ox_target', 'qb-clothes'})

    -- Extra buffer zodat NUI van resources ook klaar is
    Wait(500)
...
local function spawnLastLocation()
    ...
    -- FIX: wacht op inventory en target voordat player events gefired worden
    waitForResources({'ox_inventory', 'ox_target'})

    -- Kleine buffer voor NUI initialisatie
    Wait(300)
...
CreateThread(function()
    while true do
        ...
        if NetworkIsSessionStarted() then
            ...
            if amount == 1 then
                lib.callback.await('qbx_core:server:loadCharacter', false, characters[1].citizenid)

                if GetResourceState('qbx_apartments'):find('start') then
                    TriggerEvent('apartments:client:setupSpawnUI', characters[1].citizenid)
                elseif GetResourceState('qbx_spawn'):find('start') then
                    TriggerEvent('qb-spawn:client:setupSpawns', characters[1].citizenid)
                    TriggerEvent('qb-spawn:client:openUI', true)
                else
                    spawnLastLocation()
                end
